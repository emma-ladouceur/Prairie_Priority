...
fig_alpha <- (fig_alpha_nuts  + fig_alpha_ass + fig_alpha_inv)
...
fig_gamma_plot <- (fig_gamma_plot_nuts  + fig_gamma_plot_ass + fig_gamma_plot_inv)
...
fig_gamma_block<-(fig_gamma_block_nuts  + fig_gamma_block_ass + fig_gamma_block_inv)
...
load( '~/GRP GAZP Dropbox/Emma Ladouceur/_Projects/Prairie_Priority/Model_Fits/beta_block_rich.Rdata')
...
fig_beta_block_nuts <- ggplot() + 
  ...
  #coord_flip() +
  labs( subtitle= 'a) Nutrients'
  ) + ylab((expression(paste(italic(beta), '-Diversity', sep = ''))))
...
fig_beta_block_ass <- ggplot() + 
  ...
  labs(x = '',
       y='') +
  scale_color_manual(values =  c(	"#C0C0C0","#EC579AFF","#5A5895FF", "#15983DFF"))  + 
  ggtitle( expression(atop(paste(italic(beta),'-block-scale'),  paste('block/subplot') ) )) +
  theme_bw(base_size=18)+theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
                               #axis.text.x = element_blank(),
                               plot.margin= margin(t = 0.2, r = 0.2, b = -0.2, l = 0.2, unit = "cm"),
                               plot.title=element_text(size=18, hjust=0.5),
...
fig_beta_block <- (fig_beta_block_nuts  + fig_beta_block_ass + fig_beta_block_inv)
